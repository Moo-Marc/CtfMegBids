function [Recordings, Dataset, Message] = BidsRebuildAllFiles(...
        ...
        RemoveEmptyFields, RenameFiles, IgnoreInfoMismatch, ContinueFrom, Validate, OldRecordings)
    % Rebuild all BIDS metadata files for a CTF MEG dataset (not anatomy yet).
    %
    % If the provided BidsFolder is not the root BIDS folder, only recordings found under the
    % provided folder will be edited.
    %
    % Recreates BIDS metadata files from potentially 3 sources.  In order of priority:
    %  1) Data manually provided in BidsInfo and BidsDsInfo structures, this is matched by recording
    %  using the BidsInfo.Name (file name only with .ds) field. Since this script does not rename
    %  folders, it is not possible to change the BIDS "entities" that constitute folder names
    %  (subject ID and session ID).
    %
    %  2) Data in existing recording (not session or dataset) BIDS metadata files according to the
    %  KeepFields structure, which does not need to contain any data but only part of the field
    %  structure as returned by BidsRecordings. I.e. a subset of: KeepFields.Meg and the first level
    %  of sub-fields from the _meg.json file, KeepFields.Coordsys and the first level of sub-fields
    %  from the _coordsys.json file, .Channels (no sub-fields), and .Events (no sub-fields).  By
    %  default, it only contains KeepFields.Meg.SubjectArtefactDescription.
    %
    %  3) The metadata extracted from the raw data files.
    %
    % If SaveFiles is false (default true), no files are modified.  This can be used with Verbose =
    % true to perform a dry run and see in details what would get changed.
    %
    % If RenameFiles is true (default false), it is possible to change BIDS "entities" that are part
    % of recording file names (but not folder names), by providing them in BidsInfo. Noise or
    % resting state task names will also be standardized as per BIDS in that case. Note that
    % renaming noise recordings may break existing AssociatedEmptyRoom references, but they should
    % be found again and updated.
    %
    % If IgnoreInfoMismatch is true, missing or extra recordings provided in BidsInfo are ignored.
    %
    % ContinueFrom can be a subject index, a subject name (without "sub-") or "Scans".  This last
    % option skips all recordings and only rebuilds the scans.tsv files based on current recordings.
    %
    % If Validate is true, attempts to run the (external) BIDS validator after running. [needs to be
    % installed and a script path is currently hard coded]
    %
    % An events.json description file is created in the root BIDS folder if there are any events in
    % the dataset. Custom column descriptions can be given in the BidsDsInfo.Events field.
    %
    % Recordings can be passed in to speed up initial steps, but BidsFolder MUST be the root BIDS in
    % that case, and dataset_description.json will NOT be checked or recreated.
    %
    % Marc Lalancette, 2024-10-09

    % TODO: Make work if there are missing files.  Probably Verbose fails.

    if nargin < 12 || isempty(OldRecordings)
        OldRecordings = [];
    end
    ...
    if nargin < 4 || isempty(KeepFields)
        KeepFields = struct([]);
        % By default, keep any manually created fields.
        KeepFields.Meg.SubjectArtefactDescription = '';
        KeepFields.Meg.DewarPosition = '';
    end
    ...
    % Backup input folder if not root. This function will return the root.
    InFolder = BidsFolder;
    if isempty(OldRecordings)
    ...
    if iStart > nR % a way to skip recordings but still do scans.
    ...
    % TODO: this may not check correctly, next arrayfun may crash if iStart in between.
    ...
    if ~strcmp(InFolder, BidsFolder)
        % Only check scans files in provided sub-folder
        OldScansFiles = dir(fullfile(InFolder, '**', '*scans.tsv'));
    else
    ...
    for f = 1:numel(OldScansFiles)
        ...
        if ~ismember(OldScansFile, ScansFiles)
            % It could be an anatomical session. Ignore if only anat
            OldScansSessionContent = dir(OldScansFiles(f).folder);
            if (ismember('meg', {OldScansSessionContent.name}) || ~ismember('anat', {OldScansSessionContent.name})) && ...
                    (SaveFiles || Verbose)
    ...
    % --------------------------------------------------------
    % Dataset/Study file
    % Only if input was root BIDS folder.
    if strcmp(InFolder, BidsFolder) && ~isempty(Dataset)
        ...
        if Verbose
            NewMessage = BidsCompare(Dataset, BidsDsInfo, 'Dataset');
            if ~isempty(NewMessage)
        ...
        if Verbose
            NewMessage = BidsCompare(OldDataset, Dataset, 'Dataset');
            if ~isempty(NewMessage)
