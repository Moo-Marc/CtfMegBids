...
nodeName = matlab.lang.makeValidName(char(node.getNodeName()));
...
function sNode = ParseTree(node)
    ...
    for iAttr = 0:(attrList.getLength()-1)
        attrName = matlab.lang.makeValidName(char(attrList.item(iAttr).getName()));
    ...
    for iChild = 0:(node.getLength()-1)
        ...
        switch (node.item(iChild).getNodeType())
            case node.ELEMENT_NODE
                ...
                childName = matlab.lang.makeValidName(char(node.item(iChild).getNodeName()));
                % Read child information
                sNode.(childName) = ParseTree(node.item(iChild));
            ...
            case node.COMMENT_NODE
                nodeText = strtrim(char(node.item(iChild).getNodeValue));
                if ~isempty(nodeText)
                    sNode = nodeText; %.comment
                end
            case node.CDATA_SECTION_NODE
                nodeText = strtrim(char(node.item(iChild).getNodeValue));
                if ~isempty(nodeText)
                    sNode = nodeText; %.cdata
                end
            otherwise
